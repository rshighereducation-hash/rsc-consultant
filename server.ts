...
4. Tone & Style: Warm, intelligent, structured markdown with clean formatting, bullet points where helpful, clear explanations, and friendly guidance. Always offer to connect them directly to Mr. Rehmat Shah or senior counselors on WhatsApp (+92 334 4626284).`;
        ...
        if (reply) {
          return res.json({
            reply,
            suggestedActions: [
              { label: '💬 Chat on WhatsApp (+92 334 4626284)', actionType: 'whatsapp', payload: 'https://wa.me/923344626284?text=Hello%20RS%20Consultants!%20I%20would%20like%20further%20guidance.' },
  ...
  app.post('/api/contact', (req, res) => {
    try {
      ...
      const leadData = {
        id: applicationId || `RS-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`,
        ...
        academicBackground: academicBackground || qualification || 'Not specified',
      ...
      return res.status(200).json({
        success: true,
        message: 'Your inquiry has been successfully submitted to RS Higher Education Consultants! Our senior counselor in Peshawar will contact you within 24 hours.',
        leadId: leadData.id,
      });
    } catch (err: any) {
      ...
      return res.status(500).json({
        success: false,
        error: 'An internal error occurred while processing your request. Please contact us via WhatsApp directly.',
      });
    }
  });
  ...
  app.get('/api/export-leads-csv', (_req, res) => {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="RS_Consultants_Leads_${new Date().toISOString().slice(0, 10)}.csv"`);
  ...
  app.post('/api/consultation', (req, res) => {
    try {
      const { name, phone, email, preferredDate, preferredMode, destination, message } = req.body;
      if (!name || !phone) {
        return res.status(400).json({
          success: false,
      ...
      return res.status(200).json({
        success: true,
        message: 'Free consultation booked! An RS education advisor will reach out to confirm your time slot.',
      });
    } catch (err: any) {
      console.error('Error handling consultation booking:', err);
      return res.status(500).json({
        success: false,
        error: 'Unable to schedule consultation at this time. Please use WhatsApp for instant booking.',
      });
    }
  });
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(` RS Higher Education Consultants server running on http://localhost:${PORT}`);
  });
}
