...
export const MediaUploadModal: React.FC = () => {
  ...
  const isGermanyChecklistPdf =
    uploadModalState.type === "germany-checklist-pdf";
  const isLithuaniaChecklistPdf =
    uploadModalState.type === "lithuania-checklist-pdf";
  ...
  const processUploadedFile = (file: File) => {
    if (isPdfUpload) {
      ...
      if (!isPdf && !isImg) {
        setErrorMsg(
          "Please select a valid PDF document or image file (PDF, PNG, JPG).",
        );
        return;
      }
      ...
      if (file.size > 25 * 1024 * 1024) {
        setErrorMsg(
          "File size exceeds 25MB. Please choose a smaller PDF file.",
        );
        return;
      }
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        const maxDimension =
          isLogo || isShowcaseLogo || isLaptopLogo
            ? 800
            : isHero || isStudent
              ? 1600
              : 1200;
  ...
  const handleSave = () => {
    ...
    if (isTurkeyChecklistPdf) {
      setCustomTurkeyChecklistPdf(
        previewUrl,
        uploadedFileName || "Turkey_Checklist_Custom.pdf",
      );
      setSuccessMsg(
        "Turkey checklist PDF uploaded and saved successfully! Downloads will now serve this file.",
      );
    } else if (isTurkeyInterviewPdf) {
      setCustomTurkeyInterviewPdf(
        previewUrl,
        uploadedFileName || "Turkey_Questioner_Custom.pdf",
      );
      setSuccessMsg(
        "Turkey Questioner PDF uploaded and saved successfully! Downloads will now serve this file.",
      );
    } else if (isGermanyChecklistPdf) {
      setCustomGermanyChecklistPdf(
        previewUrl,
        uploadedFileName || "Germany_Checklist_Custom.pdf",
      );
      setSuccessMsg(
        "Germany checklist PDF uploaded and saved successfully! Downloads will now serve this file.",
      );
    } else if (isLithuaniaChecklistPdf) {
      setCustomLithuaniaChecklistPdf(
        previewUrl,
        uploadedFileName || "Lithuania_Checklist_Custom.pdf",
      );
      setSuccessMsg(
        "Lithuania checklist PDF uploaded and saved successfully! Downloads will now serve this file.",
      );
    } else if (isUkChecklistPdf) {
      setCustomUkChecklistPdf(
        previewUrl,
        uploadedFileName || "UK_Checklist_Custom.pdf",
      );
      setSuccessMsg(
        "UK checklist PDF uploaded and saved successfully! Downloads will now serve this file.",
      );
    } else if (isCyprusChecklistPdf) {
      setCustomCyprusChecklistPdf(
        previewUrl,
        uploadedFileName || "Cyprus_Checklist_Custom.pdf",
      );
      setSuccessMsg(
        "Cyprus checklist PDF uploaded and saved successfully! Downloads will now serve this file.",
      );
    } else if (isLogo) {
      setCustomLogo(previewUrl);
      setSuccessMsg(
        "Original brand logo updated successfully across the site!",
      );
    } else if (isFormLogo) {
      ...
      setSuccessMsg(
        "Universities campus architecture picture updated successfully!",
      );
      ...
      setSuccessMsg(
        "FAQ graduation & mentorship background picture updated successfully!",
      );
    } else {
  ...
  const handleResetToDefault = () => {
    if (isTurkeyChecklistPdf) {
      ...
      setUploadedFileName(null);
      setSuccessMsg(
        "Turkey checklist reset to default official generated PDF.",
      );
    } else if (isTurkeyInterviewPdf) {
      ...
      setUploadedFileName(null);
      setSuccessMsg(
        "Turkey Questioner reset to default official generated PDF.",
      );
    } else if (isGermanyChecklistPdf) {
      ...
      setUploadedFileName(null);
      setSuccessMsg(
        "Germany checklist reset to default official generated PDF.",
      );
    } else if (isLithuaniaChecklistPdf) {
      ...
      setUploadedFileName(null);
      setSuccessMsg(
        "Lithuania checklist reset to default official generated PDF.",
      );
    } else if (isUkChecklistPdf) {
      ...
      setUploadedFileName(null);
      setSuccessMsg(
        "Cyprus checklist reset to default official generated PDF.",
      );
    } else if (isLogo) {
      setCustomLogo(null);
      setPreviewUrl(null);
      setSuccessMsg(
        "Logo reset to default RS Higher Education vector insignia.",
      );
    } else if (isFormLogo) {
      setCustomFormLogo(null);
      setPreviewUrl(null);
      setSuccessMsg(
        "Form logo reset to default official RS Higher Education emblem.",
      );
    } else if (isHero) {
      setCustomHeroPhoto(null);
      setPreviewUrl(null);
      setSuccessMsg(
        "Hero picture reset to default international students photo.",
      );
    } else if (isStudent) {
      setCustomStudentBanner(null);
      setPreviewUrl(null);
      setSuccessMsg(
        "Student picture reset to default international campus photograph.",
      );
    } else if (isSuccess) {
      setCustomSuccessBanner(null);
      setPreviewUrl(null);
      setSuccessMsg(
        "Student success picture reset to default graduates photograph.",
      );
    } else if (isServices) {
      setCustomServicesBanner(null);
      setPreviewUrl(null);
      setSuccessMsg(
        "Services picture reset to default students study session photograph.",
      );
    } else if (isLaptopLogo) {
      ...
      setPreviewUrl(null);
      setSuccessMsg(
        "Universities picture reset to default global campuses architecture photograph.",
      );
    } else if (isShowcaseLogo) {
      ...
      setPreviewUrl(null);
      setSuccessMsg(
        "FAQ graduation background picture reset to official convocation photo.",
      );
    } else {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="inline-flex items-center gap-1.5 text-[11px] font-bold text-[#DB0303] uppercase tracking-wider font-heading">
              ...
              <span>
                {isPdfUpload ? "Upload PDF from PC" : "Import Asset from PC"}
              </span>
            </div>
          ...
          <div className="space-y-3 flex flex-col items-center">
            ...
            <div className="space-y-1">
              <p className="text-sm font-bold text-slate-800 font-heading">
                {isPdfUpload
                  ? "Click to browse PDF from your PC or drag & drop here"
                  : "Click to browse or drag & drop image here"}
              </p>
              <p className="text-xs text-slate-500">
                {isPdfUpload
                  ? "Supports official .PDF files or scans (up to 25MB)"
                  : "Supports PNG, JPG, JPEG, SVG, WEBP (up to 15MB)"}
              </p>
            </div>
        ...
        {(previewUrl || currentAsset) && (
          <div className="p-4 rounded-2xl bg-slate-100 border border-slate-200 space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-700 font-bold font-heading">
              <span>
                {previewUrl ? "Selected New File:" : "Current Active File:"}
              </span>
              {previewUrl && (
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500">
                      {uploadedFileSize
                        ...
                        : "Custom document ready for student download"}
                    </p>
                  </div>
                </div>
        ...
        <div className="space-y-2 pt-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={
                isPdfUpload
                  ? "https://example.com/official-turkey-checklist.pdf"
                  : "https://example.com/my-original-logo.png"
              }
              value={urlInput}
        ...
        <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3">
          {currentAsset ? (
            <button
              ...
              <span>
                Reset to Default{" "}
                {isPdfUpload ? "Generated PDF" : isLogo ? "Logo" : "Photo"}
              </span>
            </button>
          ) : (
            <div />
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            ...
            <button
              ...
              <span>
                {isProcessing ? "Processing..." : "Apply & Save Document"}
              </span>
            </button>
          </div>
        </div>
