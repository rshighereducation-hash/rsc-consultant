...
const STUDY_LEVELS = [
  {
    label: "Bachelor's / Undergraduate",
    value: "Undergraduate (Bachelor - 3 to 4 Yrs)",
    icon: "🎓",
  },
  {
    label: "Master's / Postgraduate",
    value: "Master / Postgraduate (1 to 2 Yrs)",
    icon: "🏛️",
  },
  {
    label: "MBBS / MD Medicine",
    value: "Medicine / Dentistry (MD / MBBS)",
    icon: "🩺",
  },
  { label: "PhD / Doctorate", value: "PhD / Doctorate", icon: "🔬" },
  {
    label: "Foundation / Pathway",
    value: "Foundation / Pre-Master Pathway",
    icon: "📚",
  },
];
 ...
 */
export const FormLogoUploader: React.FC<{ height?: number }> = ({
  height = 85,
}) => {
...
export const ContactForm: React.FC<ContactFormProps> = ({
  ...
  const { customFormLogo, customLogo, setCustomFormLogo, openFormLogoModal } =
    useBrand();
  const activeLogo = customFormLogo || customLogo;
  ...
  const [financialBracket, setFinancialBracket] = useState(
    "Self Funded / Family Sponsorship",
  );
  const [academicBackground, setAcademicBackground] = useState("");
  const [workExperience, setWorkExperience] = useState(
    "None / Direct Progression",
  );
  ...
  const [academicRows, setAcademicRows] = useState<AcademicRecord[]>([
    {
      degree: "4-Year Bachelor (BS / BBA / BE / CS)",
      year: "2024",
      grade: "3.25 CGPA (81.25%)",
  ...
  const calculateAcademicScore = () => {
    if (calcMode === "marks") {
      ...
      if (pct >= 80) {
        ...
        scholarshipChance =
          "Direct Admission in UK, Germany, Lithuania, Turkey";
      } else if (pct >= 60) {
        grade = "B";
        ...
        scholarshipChance =
          "Eligible for Cyprus, Turkey, UK International Year 1";
      } else {
        grade = "D";
        remark = "Pass";
      ...
      if (cgpa >= 3.6) {
        ...
        scholarshipChance =
          "High Chance for Top Ranked UK & German Public Universities";
      } else if (cgpa >= 3.0) {
        grade = "A (Good)";
        remark = "First Class (71% - 84% HEC)";
        scholarshipChance =
          "Eligible for Direct Master in UK, Germany, Lithuania, Turkey";
      } else if (cgpa >= 2.5) {
        grade = "B (Average)";
        remark = "Upper Second Class (60% - 70% HEC)";
        scholarshipChance =
          "Eligible for UK, Cyprus, Lithuania, Turkey Universities";
      } else if (cgpa >= 2.0) {
        grade = "C (Pass)";
        remark = "Pass (50% - 59% HEC)";
        scholarshipChance =
          "Eligible for Pre-Master / Cyprus & European Pathways";
      } else {
        grade = "F";
    ...
    setAcademicRows([
      {
        ...
        grade:
          calcMode === "cgpa"
            ? `${calcObtainedCgpa} CGPA (${score.percentage})`
  ...
  const [lastPayload, setLastPayload] = useState<ApplicationPdfData | null>(
    null,
  );
  const [showDetailedAcademicFields, setShowDetailedAcademicFields] =
    useState(false);

  const [syncStatus, setSyncStatus] = useState<string | null>(null);

  const normalizedDestSlug =
    preferredDestination === "turkey"
      ? "turkiye"
      : preferredDestination === "cyprus"
        ? "south-cyprus"
        : preferredDestination;
  const selectedDestObj = ALL_DESTINATIONS.find(
    (d) =>
      d.slug === normalizedDestSlug ||
      d.id === normalizedDestSlug ||
      d.slug === preferredDestination,
  );
  const destinationDisplayName =
    preferredDestination === "other"
      ? customOtherCountry.trim()
        ? `Other (${customOtherCountry.trim()})`
        : "Other Country"
      : selectedDestObj
        ? selectedDestObj.countryName === "Türkiye"
          ? "Turkey (Türkiye)"
          : selectedDestObj.countryName
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validAcademicRows = academicRows.filter(
      (r) => r.degree.trim() || r.grade.trim() || r.institute.trim(),
    );
    ...
    const leadPayload: ApplicationPdfData = {
      ...
      city: city.trim(),
      currentAddress:
        currentAddress.trim() || (city ? `${city}, Pakistan` : "Peshawar, KPK"),
      permanentAddress:
        permanentAddress.trim() ||
        currentAddress.trim() ||
        (city ? `${city}, Pakistan` : "Peshawar, KPK"),
      destination: destinationDisplayName,
      preferredCountries: destinationDisplayName,
      studyLevel,
      preferredCourse:
        preferredCourse.trim() ||
        academicBackground.trim() ||
        "Business, Computing, Engineering or Health Sciences",
      targetIntake,
      ieltsStatus,
      ...
      financialBracket,
      academicRows:
        validAcademicRows.length > 0 ? validAcademicRows : undefined,
      academicBackground:
        academicBackground.trim() ||
        (validAcademicRows[0]
          ? `${validAcademicRows[0].degree} (${validAcademicRows[0].grade})`
      ...
      additionalNotes:
        message.trim() ||
        "Free profile evaluation and university admission guidance requested.",
    ...
    let pdfBase64 = "";
    const sanitizedName = (leadPayload.fullName || "Student").replace(
      /[^a-zA-Z0-9]/g,
      "_",
    );
    ...
    try {
      const pdfFile =
        PdfGenerationService.generateApplicationPdfFile(leadPayload);
  ...
  const handleSendPdfToWhatsApp = async () => {
    ...
    try {
      const pdfFile =
        PdfGenerationService.generateApplicationPdfFile(lastPayload);
      if (navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
      ...
      console.warn(
        "Native PDF file share not supported or cancelled:",
        shareErr,
      );
    }
  ...
  if (submitted && lastPayload) {
    const displayDate =
      lastPayload.submittedAt || new Date().toLocaleDateString("en-GB");
            ...
            <div className="flex items-center gap-3.5 text-left">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600">
                  Student Referral No:{" "}
                  <span className="font-mono font-bold text-[#A30000]">
                    {appId}
                  </span>{" "}
                  • File:{" "}
                  <span className="font-mono font-semibold text-slate-800">
                    RS_Inquiry_Form_
                    {(lastPayload.fullName || "Student").replace(
                      /[^a-zA-Z0-9]/g,
                      "_",
                    )}
                    .pdf
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs text-emerald-950">
              <div className="flex items-start gap-2 bg-white/80 p-2.5 rounded-xl border border-emerald-100">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-slate-600">
                    The PDF has been saved in your device's Downloads folder.
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2 bg-white/80 p-2.5 rounded-xl border border-emerald-100">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-slate-600">
                    Click "Send PDF File to WhatsApp" to open WhatsApp chat.
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2 bg-white/80 p-2.5 rounded-xl border border-emerald-100">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-slate-600">
                    In WhatsApp, tap 📎 &gt; Document, select your PDF file, and
                    send!
                  </span>
                </div>
              </div>
          ...
          <div className="border border-slate-300 p-5 sm:p-8 rounded-2xl relative space-y-6">
            ...
            <div className="text-center space-y-1 pt-2">
              ...
              <p className="text-[11px] sm:text-xs text-slate-600 font-medium italic">
                Please fill out the form below and you will be receiving free
                consultation suitable to your needs.
              </p>
            </div>
                ...
                <table className="w-full text-xs text-left border-collapse">
                  <thead>
                    <tr className="bg-[#C59B4E] text-white font-bold border-b border-amber-600">
                      <th className="py-2 px-3 border-r border-amber-600/50 text-center w-[28%]">
                        Degree / Course
                      </th>
                      <th className="py-2 px-3 border-r border-amber-600/50 text-center w-[15%]">
                        Year
                      </th>
                      <th className="py-2 px-3 border-r border-amber-600/50 text-center w-[25%]">
                        Grade / CGPA / %age
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {(lastPayload.academicRows &&
                    lastPayload.academicRows.length > 0
                      ? lastPayload.academicRows
                      : academicRows
                    ).map((row, idx) => (
                      <tr
              ...
              <div className="space-y-2.5 text-xs pt-1 text-slate-800">
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <div className="grow border-b border-slate-500 pb-0.5 font-black text-[#A30000] px-1">
                    {lastPayload.destination ||
                      lastPayload.preferredCountries ||
                      "UK / Europe"}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                  ...
                  <label className="flex items-center gap-1 cursor-pointer">
                    <input
                      type="checkbox"
                      readOnly
                      checked={(lastPayload.ieltsStatus || "")
                        .toLowerCase()
                        .includes("ielts")}
                      className="accent-[#A30000] rounded"
                    />
                    <span>IELTS</span>
                  </label>
                  <label className="flex items-center gap-1 cursor-pointer">
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <div className="grow border-b border-slate-500 pb-0.5 font-semibold text-slate-900 px-1">
                    {lastPayload.preferredCourse ||
                      lastPayload.academicBackground ||
                      "Business, Computing, Engineering or Health Sciences"}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                  ...
                  <label className="flex items-center gap-1 cursor-pointer">
                    <input
                      type="checkbox"
                      readOnly
                      checked={(lastPayload.targetIntake || "")
                        .toLowerCase()
                        .includes("january")}
                      className="accent-[#A30000] rounded"
                    />
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <div className="grow border-b border-slate-500 pb-0.5 font-medium text-slate-800 px-1">
                    {lastPayload.workExperience ||
                      "N/A (Direct Academic Progression)"}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                  ...
                  <label className="flex items-center gap-1 cursor-pointer">
                    <input
                      type="checkbox"
                      readOnly
                      checked={(lastPayload.hearAboutUs || "")
                        .toLowerCase()
                        .includes("social")}
                      className="accent-[#A30000] rounded"
                    />
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <div className="grow border-b border-slate-500 pb-0.5 font-medium text-slate-800 px-1">
                    {lastPayload.additionalNotes ||
                      lastPayload.message ||
                      "Free initial profile assessment requested."}
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="space-y-1.5 text-[11px] text-slate-800">
                    <div>
                      ...
                      <p className="text-slate-600 pl-3">
                        – Photo and Signature Page (Scan Copy)
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-slate-600 pl-3">
                        – Two reference letters, not older than 6 months
                      </p>
                    </div>
                    <div>
                      <span className="font-bold">
                      ...
                      <p className="text-slate-600 pl-3">
                        – Required in soft copy (scanned) as well
                      </p>
                    </div>
                    <div>
                      <span className="font-bold">☐ Contact Information</span>
                      <p className="text-slate-600 pl-3">
                        – Valid Contact Number & Email Address
                      </p>
                    </div>
                    <div>
                      <span className="font-bold">
                      ...
                      <p className="text-slate-600 pl-3">
                        – IELTS / TOEFL / PTE (if available)
                      </p>
                    </div>
                    <div>
                      <span className="font-bold">☐ Updated CV</span>
                      <p className="text-slate-600 pl-3">
                        – In Europass CV format
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-slate-300 rounded-xl p-3 bg-slate-50/50 space-y-2.5">
                  ...
                  <div className="space-y-2 text-[11px]">
                    ...
                    <div className="p-2 rounded-lg bg-white border border-slate-200 space-y-1">
                      <div className="font-bold text-[#A30000]">
                        Matric DMC & Certificate
                      </div>
      ...
      <div className="flex flex-col items-center justify-center text-center gap-3 pb-6 border-b border-slate-100">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 font-medium">
            RS Higher Education Consultants • Head Office: Office No. UG-389,
        ...
        <div className="space-y-2">
            ...
            <select
              ...
              {ALL_DESTINATIONS.map((d) => (
                <option key={d.slug} value={d.slug}>
                  {d.flagEmoji}{" "}
                  {d.countryName === "Türkiye"
                    ? "Turkey (Türkiye)"
                    : d.countryName}
                </option>
              ))}
              <option value="other">
                🌍 Other Country (Specify Custom Destination)
              </option>
            </select>
          </div>
          ...
          {preferredDestination === "other" && (
            <div className="p-3.5 bg-red-50/70 border border-red-200 rounded-2xl space-y-2 animate-in fade-in slide-in-from-top-2 duration-200 shadow-2xs">
              <label
                htmlFor="custom-other-country"
                className="text-xs font-bold text-[#DB0303] flex items-center justify-between font-heading">
                <span className="flex items-center gap-1.5">
              ...
              <p className="text-[11px] text-slate-600 leading-snug">
                RS Higher Education Consultants provides comprehensive guidance,
                document evaluation, and university admissions for all
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              <option value="September 2026 (Fall Intake)">
                September 2026 (Fall Intake)
              </option>
              <option value="January 2027 (Spring Intake)">
                January 2027 (Spring Intake)
              </option>
              <option value="May / Summer 2027">May / Summer 2027</option>
              <option value="Immediate Upcoming Intake">
                Immediate Upcoming Intake
              </option>
            </select>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="w-full bg-slate-50/80 border border-slate-200 rounded-2xl px-4 py-3 text-xs sm:text-sm text-slate-900 font-medium focus:bg-white focus:outline-none focus:border-[#DB0303] transition-all shadow-xs">
              <option value="Planning to take IELTS/PTE">
                Planning to take IELTS/PTE
              </option>
              <option value="Already have IELTS/PTE score">
                Already have IELTS/PTE score
              </option>
              <option value="Looking for IELTS Waiver / MOI">
                Looking for IELTS Waiver / MOI
              </option>
              <option value="Need English Test Coaching">
                Need English Test Coaching
              </option>
            </select>
          </div>
        </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-300">
                  Enter your latest qualification and score. Percentage, grade,
                  and admission standing are computed and auto-filled in
                  real-time.
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="space-y-1 sm:col-span-2">
              ...
              <select
                ...
                {calcMode === "cgpa" ? (
                  <>
                    <option value="4-Year Bachelor (BS / BBA / BE / CS)">
                      4-Year Bachelor (BS / BBA / BE / CS)
                    </option>
                    <option value="2-Year Master (MS / MPhil / MBA)">
                      2-Year Master (MS / MPhil / MBA)
                    </option>
                    <option value="MBBS / BDS / Pharm-D / DPT">
                      MBBS / BDS / Pharm-D / DPT
                    </option>
                    <option value="Associate Degree / 2-Year BA / BSc">
                      Associate Degree / 2-Year BA / BSc
                    </option>
                    <option value="Postgraduate Diploma / Other Degree">
                      Postgraduate Diploma / Other Degree
                    </option>
                  </>
                ) : (
                  <>
                    <option value="Intermediate / FSc (Pre-Medical)">
                      Intermediate / FSc (Pre-Medical)
                    </option>
                    <option value="Intermediate / FSc (Pre-Engineering)">
                      Intermediate / FSc (Pre-Engineering)
                    </option>
                    <option value="Intermediate / ICS (Computer Science)">
                      Intermediate / ICS (Computer Science)
                    </option>
                    <option value="Intermediate / I.Com / F.A">
                      Intermediate / I.Com / F.A
                    </option>
                    <option value="A-Levels / Cambridge High School">
                      A-Levels / Cambridge High School
                    </option>
                    <option value="Matriculation / O-Levels">
                      Matriculation / O-Levels
                    </option>
                    <option value="DAE / 3-Year Technical Diploma">
                      DAE / 3-Year Technical Diploma
                    </option>
                  </>
                )}
              </select>
          ...
          <div className="bg-slate-950/90 rounded-xl p-3.5 border border-slate-700/80 flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              {calculatedScore.germanGrade && (
                <div
                  className="bg-slate-800/90 px-3 py-1.5 rounded-lg border border-slate-700 text-center"
                  title="German Bavarian Grade Equivalent (Uni-Assist)">
                  <span className="text-[9px] text-slate-400 uppercase font-bold block">
                    German Grade
                  </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="w-full bg-slate-50/80 border border-slate-200 rounded-2xl px-4 py-3 text-xs sm:text-sm text-slate-900 font-medium focus:bg-white focus:outline-none focus:border-[#DB0303] transition-all shadow-xs">
              <option value="Self Funded / Family Sponsorship">
                Self Funded / Family Sponsorship
              </option>
              <option value="Seeking Partial University Scholarship">
                Seeking Partial University Scholarship
              </option>
              <option value="Bank Statement Ready (Rs. 8M - 15M)">
                Bank Statement Ready (Rs. 8M - 15M)
              </option>
              <option value="Low Tuition Budget (Under £12,000 / $15,000)">
                Low Tuition Budget (Under £12,000 / $15,000)
              </option>
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="w-full bg-slate-50/80 border border-slate-200 rounded-2xl px-4 py-3 text-xs sm:text-sm text-slate-900 font-medium focus:bg-white focus:outline-none focus:border-[#DB0303] transition-all shadow-xs">
              <option value="Social Media (Facebook / Instagram / TikTok)">
                Social Media (Facebook / Instagram / TikTok)
              </option>
              <option value="Friend or Student Referral">
                Friend or Student Referral
              </option>
              <option value="Google Search / Website">
                Google Search / Website
              </option>
              <option value="Deans Trade Centre Walk-in">
                Deans Trade Centre Walk-in
              </option>
            </select>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-slate-700 font-heading flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400 font-normal">
              Scholarships, Dependent Visa, Gap justification
            </span>
          </label>
          <textarea
            id="form-message"
