...
export const DestinationDetailView: React.FC<DestinationDetailViewProps> = ({
  ...
  const isTurkey =
    destination.slug === "turkey" ||
    destination.slug === "turkiye" ||
    destination.slug === "türkiye";
  const isCyprus =
    destination.slug === "south-cyprus" || destination.slug === "cyprus";
  const isUk =
    destination.slug === "united-kingdom" || destination.slug === "uk";
  ...
  const handleDownloadGermanyPdf = () => {
    ...
    try {
      if (customGermanyChecklistPdf) {
        ...
        a.download =
          customGermanyChecklistName ||
          "Germany_Checklist_RS_Higher_Education.pdf";
  ...
  const handleDownloadTurkeyPdf = () => {
    ...
    try {
      if (customTurkeyChecklistPdf) {
        ...
        a.download =
          customTurkeyChecklistName ||
          "Turkey_Checklist_RS_Higher_Education.pdf";
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
      } else {
  ...
  const handleDownloadTurkeyInterviewPdf = () => {
    ...
    try {
      if (customTurkeyInterviewPdf) {
        ...
        a.download =
          customTurkeyInterviewName ||
          "Turkey_Questioner_RS_Higher_Education.pdf";
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
      } else {
  ...
  const undergradReq = destination.entryRequirements?.undergraduate || {
    academic:
      destination.admissionRequirements?.academic ||
      "Minimum 60% in Intermediate / A-Levels.",
    english:
      destination.admissionRequirements?.english ||
      "IELTS 6.0 / PTE 54 / MOI accepted.",
    documents: destination.admissionRequirements?.documents || [],
    notes: destination.admissionRequirements?.notes || "",
    specialNotes:
      destination.admissionRequirements?.specialNotes ||
      destination.admissionRequirements?.notes ||
      "",
  };
  ...
  const postgradReq = destination.entryRequirements?.postgraduate || {
    academic:
      destination.admissionRequirements?.academic ||
      "4-year Bachelor degree with 2.5+ CGPA from HEC recognized university.",
    english:
      destination.admissionRequirements?.english ||
      "IELTS 6.5 / PTE 58 / MOI where applicable.",
    documents: destination.admissionRequirements?.documents || [],
    notes: destination.admissionRequirements?.notes || "",
    specialNotes:
      destination.admissionRequirements?.specialNotes ||
  ...
  const visaDocs = destination.visaProcess?.documentChecklist ||
    destination.visaProcess?.requiredDocuments || [
  ...
  const officialVisaLink =
    destination.visaProcess?.officialLink ||
    destination.visaProcess?.officialPortalUrl ||
    destination.officialGovernmentLinks?.[0]?.url ||
    "https://rshec.pk";
  const officialAuthority =
    destination.visaProcess?.officialAuthority ||
    destination.officialGovernmentLinks?.[0]?.agency ||
    destination.visaProcess?.name ||
    `${destination.countryName} Immigration Authority`;

  return (
    <div
      id={`destination-page-${destination.slug}`}
      className="min-h-screen bg-slate-50/70 pt-28 pb-20">
              ...
              <div className="pt-2 flex flex-wrap items-center gap-3">
                ...
                {isGermany && (
                  <button
                    ...
                    className="px-6 py-4 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-black text-xs sm:text-sm rounded-2xl font-heading shadow-md transition-all flex items-center gap-2 cursor-pointer hover:scale-[1.02] border border-amber-300 active:scale-95"
                    title={
                      customGermanyChecklistPdf
                        ? `Download custom: ${customGermanyChecklistName}`
                        : "Download Official Study in Germany University & Embassy Visa Checklist PDF"
                    }>
                    <Download
                      className={`w-4 h-4 text-slate-950 ${isDownloadingPdf ? "animate-bounce" : ""}`}
                    />
                    <span>
                      {isDownloadingPdf
                        ? "Preparing PDF..."
                        : "Download Germany Checklist (PDF)"}
                    </span>
                    {customGermanyChecklistPdf && (
                ...
                {isTurkey && (
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <button
                      ...
                      className="px-5 py-3.5 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-black text-xs sm:text-sm rounded-2xl font-heading shadow-md transition-all flex items-center gap-2 cursor-pointer hover:scale-[1.02] border border-amber-300 active:scale-95"
                      title={
                        customTurkeyChecklistPdf
                          ? `Download custom: ${customTurkeyChecklistName}`
                          : "Download Turkey checklist PDF"
                      }>
                      <Download
                        className={`w-4 h-4 text-slate-950 ${isDownloadingPdf ? "animate-bounce" : ""}`}
                      />
                    ...
                    <button
                      ...
                      className="px-5 py-3.5 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-black text-xs sm:text-sm rounded-2xl font-heading shadow-md transition-all flex items-center gap-2 cursor-pointer hover:scale-[1.02] border border-red-500 active:scale-95"
                      title={
                        customTurkeyInterviewPdf
                          ? `Download custom: ${customTurkeyInterviewName}`
                          : "Download Turkey Questioner PDF"
                      }>
                      <FileText
                        className={`w-4 h-4 text-white ${isDownloadingPdf ? "animate-bounce" : ""}`}
                      />
                      <span>
                        {isDownloadingPdf
                          ? "Preparing PDF..."
                          : "Turkey Questioner (PDF)"}
                      </span>
                      {customTurkeyInterviewPdf && (
              ...
              <div className="space-y-3 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-slate-900">
                    {universitiesList.length} Institutions
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-medium">
                  ...
                  <span className="font-bold text-slate-900">
                    {intakesList[0]?.month || "Fall / September"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-medium">
                  ...
                  <span className="font-bold text-slate-900">
                    {destination.estimatedCosts?.tuitionUndergrad || "Varies"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-medium">
                  ...
                  <span className="font-bold text-slate-900 truncate max-w-[150px]">
                    {destination.workOpportunities?.duringStudy?.split(
                      ";",
                    )?.[0] || "20 hrs/week permitted"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-medium">
                  ...
                  <span className="font-bold text-[#DB0303] truncate max-w-[150px]">
                    {destination.workOpportunities?.postStudyWork ||
                      destination.workOpportunities?.postStudy ||
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 relative z-10 space-y-10">
        ...
        <div className="bg-white p-2 rounded-2xl border border-slate-200/80 shadow-md shadow-slate-900/5 flex flex-wrap gap-2 justify-center sm:justify-start">
          {[
            ...
            {
              id: "universities",
              label: `10 Featured Universities (${universitiesList.length})`,
            },
          ].map((tab) => (
            <button
              key={tab.id}
          ...
          <div className="space-y-8 animate-in fade-in duration-200">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-sm space-y-6">
              <div className="space-y-1">
                ...
                <h2 className="text-2xl font-black text-slate-900 font-heading">
                  Why Pakistani Students Choose to Study in{" "}
                  {destination.countryName}
                </h2>
              </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-sm space-y-6">
              <div className="space-y-1">
                ...
                <h2 className="text-2xl font-black text-slate-900 font-heading">
                  Popular Study Areas & In-Demand Programs in{" "}
                  {destination.countryName}
                </h2>
              </div>
                ...
                {popularFieldsList.map((field, fIdx) => (
                  <div
                    ...
                    <p className="text-xs text-slate-600">
                      {field.desc ||
                        field.description ||
                        "Comprehensive curriculum with high industry demand."}
                    </p>
            ...
            <div className="bg-white text-slate-900 rounded-3xl p-6 sm:p-10 border border-red-100 shadow-xl shadow-red-950/5 space-y-6">
              <div className="space-y-1">
                ...
                <h2 className="text-2xl font-black font-heading text-slate-900">
                  Work Opportunities & Post-Study Visas in{" "}
                  {destination.countryName}
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs sm:text-sm">
                ...
                <div className="p-5 rounded-2xl bg-red-50/40 border border-red-100 space-y-2">
                  ...
                  <p className="text-slate-600 leading-relaxed">
                    {destination.workOpportunities?.jobMarket ||
                      destination.workOpportunities?.notes ||
                      "STEM, Software Engineering, Healthcare, Finance, and Engineering disciplines."}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-sm space-y-6">
              <div className="space-y-1">
                ...
                <p className="text-xs sm:text-sm text-slate-600">
                  Equivalent grade benchmarks for Intermediate (FSc / ICS / ICom
                  / FA), A-Levels, and 4-Year Bachelor degrees.
                </p>
              </div>
                ...
                <div className="p-6 rounded-2xl bg-slate-50/80 border border-slate-200 space-y-3">
                  ...
                  <div className="space-y-2 text-xs sm:text-sm text-slate-700">
                    <p>
                      ...
                      {undergradReq.academic}
                    </p>
                    <p>
                      <strong>English Proficiency (IELTS/PTE):</strong>{" "}
                      {undergradReq.english}
                    </p>
              ...
              <div className="p-6 rounded-2xl bg-red-50/40 border border-red-100 space-y-3">
                <h4 className="text-sm font-bold text-slate-900 font-heading flex items-center gap-2">
                  ...
                  <span>
                    Mandatory Academic Documents for Pakistani Students:
                  </span>
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-700 font-medium">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Matric & Intermediate Board Transcripts / Certificates
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="w-3.5 h-3.5 text-[#DB0303] shrink-0" />
                    <span>
                      Official Bachelor Degree & Detailed Marks Certificates
                      (DMCs)
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900 to-[#7A0000] text-white flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl border-2 border-red-500/30">
                  <div className="space-y-1 text-center sm:text-left">
                    ...
                    <p className="text-xs text-slate-300">
                      Attestation paths (IBCC / HEC / MOFA), Uni-Assist fees
                      (€75/€30), and Blocked Account requirements (€11,904/yr).
                    </p>
                  </div>
              ...
              {isTurkey && (
                <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900 to-[#7A0000] text-white flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl border-2 border-red-500/30">
                  <div className="space-y-1 text-center sm:text-left">
                    ...
                    <p className="text-xs text-slate-300">
                      Admission requirements, Anatolia visa process, 24 partner
                      universities, and 18 visa interview Q&amp;As.
                    </p>
                  </div>
        ...
        {activeTab === "process" && (
          <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-sm space-y-6 animate-in fade-in duration-200">
            <div className="space-y-1">
              ...
              <p className="text-xs sm:text-sm text-slate-600">
                Follow this exact step-by-step procedure managed by RS Higher
            ...
            <div className="pt-6 border-t border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {intakesList.map((intake, iIdx) => (
                  <div
                    key={iIdx}
                    className="p-4.5 rounded-2xl bg-red-50/30 border border-red-100 space-y-1.5">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-[#DB0303] font-heading">
                      <Calendar className="w-3.5 h-3.5" />
                      <span>
                        {intake.term || intake.name || `Intake 0${iIdx + 1}`}
                      </span>
                    </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
                  ...
                  <p className="text-base font-bold text-slate-900 font-heading">
                    {destination.estimatedCosts?.tuitionUndergrad ||
                      "Competitive rates"}
                  </p>
                ...
                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
                  ...
                  <p className="text-base font-bold text-slate-900 font-heading">
                    {destination.estimatedCosts?.tuitionPostgrad ||
                      "Competitive rates"}
                  </p>
                  <p className="text-[11px] text-slate-400">
                    per academic year
                ...
                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
                  ...
                  <p className="text-base font-bold text-slate-900 font-heading">
                    {destination.estimatedCosts?.livingMonthly ||
                      destination.estimatedCosts?.livingExpenses ||
                      "Affordable"}
                  </p>
                  <p className="text-[11px] text-slate-400">
                    including food & rent
                ...
                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
                  ...
                  <p className="text-base font-bold text-slate-900 font-heading">
                    {destination.estimatedCosts?.healthInsurance ||
                      destination.estimatedCosts?.visaFee ||
                      "Standard student fee"}
                  </p>
                  <p className="text-[11px] text-slate-400">
                    mandatory coverage
                  </p>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-sm space-y-6">
              <div className="space-y-1">
                ...
                <h2 className="text-2xl font-black text-slate-900 font-heading">
                  Scholarships Available for Pakistani Students in{" "}
                  {destination.countryName}
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {scholarshipsList.map((sch, sIdx) => (
                  <div
                    key={sIdx}
                    className="p-5 rounded-2xl bg-slate-50/70 border border-slate-200/80 space-y-2 hover:border-red-200 transition-colors">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-[#DB0303] uppercase tracking-wider font-heading">
                    ...
                    <p className="text-xs text-slate-600 leading-relaxed">
                      {sch.criteria ||
                        sch.description ||
                        "Merit and academic achievement-based qualification."}
                    </p>
                  </div>
                ))}
              </div>
        ...
        {activeTab === "visa" && (
          <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-sm space-y-6 animate-in fade-in duration-200">
            <div className="space-y-1">
              ...
              <p className="text-xs sm:text-sm text-slate-600">
                Official document verification, financial sponsorship proof, and
                consular interview protocols.
              </p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-50/80 border border-slate-200 space-y-2">
              ...
              <p className="text-xs sm:text-sm text-slate-700 leading-relaxed">
                ...
                {destination.visaProcess?.name ||
                  `${destination.countryName} National Student Visa`}{" "}
                | <strong>Processing Time:</strong>{" "}
                {destination.visaProcess?.processingTime || "3 to 6 weeks"}
              </p>
              <p className="text-xs text-slate-600">
                <strong>Financial Proof:</strong>{" "}
                {destination.visaProcess?.financialProof ||
                  "Valid bank statement or blocked account demonstrating sufficient living and tuition funds."}
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {visaDocs.map((doc, dIdx) => (
                  <div
                    key={dIdx}
                    className="flex items-start gap-2.5 p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-xs text-slate-700 font-medium">
              ...
              <div className="p-6 rounded-2xl bg-red-50 border-2 border-red-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="space-y-1 text-center sm:text-left">
                  <h4 className="text-sm font-bold font-heading text-slate-900 flex items-center justify-center sm:justify-start gap-2">
                    <FileCheck2 className="w-4 h-4 text-[#DB0303]" />
                    <span>
                      Printable 2026 German Visa & Blocked Account Checklist
                    </span>
                  </h4>
                  <p className="text-xs text-slate-600">
                    Includes Sperrkonto deposit details (€11,904/yr),
                    Fintiba/Expatrio guidelines, waiting list protocols, and
                    attested documents list.
                  </p>
                </div>
                <button
                  onClick={handleDownloadGermanyPdf}
              ...
              <div className="p-6 rounded-2xl bg-red-50 border-2 border-red-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="space-y-1 text-center sm:text-left">
                  <h4 className="text-sm font-bold font-heading text-slate-900 flex items-center justify-center sm:justify-start gap-2">
                    <FileCheck2 className="w-4 h-4 text-[#DB0303]" />
                    <span>
                      Printable 2026 Türkiye Visa Checklist &amp; Interview
                      Questioner
                    </span>
                  </h4>
                  <p className="text-xs text-slate-600">
                    Includes Anatolia visa requirements, $1,000 deposit, $235
                    Anatolia fee, and 18 consulate interview model Q&amp;As.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                  <button
        ...
        {activeTab === "universities" && (
          <div className="space-y-6 animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-sm space-y-2">
              ...
              <p className="text-xs sm:text-sm text-slate-600">
                Official higher education institutions with high acceptance
                rates for qualified Pakistani students.
              </p>
            </div>
                  ...
                  <div className="space-y-2.5">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3.5">
                        <UniLogoBadge
                          websiteUrl={uni.officialWebsite || uni.websiteUrl}
                          name={uni.name}
                        />
                        <div>
                    ...
                    <p className="text-xs text-slate-600">
                      {uni.shortDescription ||
                        "Accredited institution offering globally recognized undergraduate and postgraduate degrees."}
                    </p>
        ...
        <div className="p-8 sm:p-10 bg-gradient-to-r from-[#DB0303] to-[#B30000] rounded-3xl text-white shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="space-y-2 text-center md:text-left">
            ...
            <p className="text-xs sm:text-sm text-red-100 max-w-xl font-medium">
              Our Peshawar counseling team is ready to evaluate your transcripts
              and structure your admission file today.
            </p>
          </div>
